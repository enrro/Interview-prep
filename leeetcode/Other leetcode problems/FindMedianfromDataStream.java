...
class MedianFinder {
    ...
    public MedianFinder() {
        ...
        smallest = new PriorityQueue<>(Collections.reverseOrder());
    }
    
    public void addNum(int num) {
        largest.offer((long)num);
        smallest.offer(largest.poll());
        if(smallest.size() > largest.size()){
            largest.offer(smallest.poll());
        }
    }
    
    public double findMedian() {
        return largest.size() == smallest.size() ? (largest.peek() + smallest.peek()) / 2.0 : largest.peek();
    }
}
